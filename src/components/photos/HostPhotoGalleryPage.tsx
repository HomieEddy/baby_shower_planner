  ...
  return (
    <div className="min-h-screen bg-[#FAF6F0] py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Modal open={isTableQrModalOpen} onClose={() => setIsTableQrModalOpen(false)} maxWidth="2xl"
            ...
            <div className="print:hidden">
              ...
              <div className="space-y-6">
                {printScope === 'single' && selectedPrintTable && (
                  <div className="printable-qr-card p-6 bg-[#FAF6F0] rounded-3xl border-2 border-dashed border-[#8B735B] text-center space-y-4">
                    <span className="inline-block px-3 py-1 rounded-full bg-amber-200 text-amber-900 text-[10px] font-bold uppercase tracking-wider">
                      {selectedPrintTable.name} • Photo Drop
                    </span>
                    ...
                    <div className="w-40 h-40 mx-auto bg-white p-3 rounded-2xl border-2 border-[#CBAE94] shadow-md flex items-center justify-center">
                      <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(
                          `${window.location.origin}/upload-photos?tableId=${selectedPrintTable.id}`
                        )}`}
                        alt={`QR code for ${selectedPrintTable.name}`}
                    ...
                    <div className="text-[10px] font-mono text-[#8B735B] pt-2 border-t border-dashed border-[#CBAE94]/60">
                      Table: {selectedPrintTable.name} • Scan & Upload • No App Required
                    </div>
                  </div>
                )}
                    ...
                    {tables.map((tbl, idx) => (
                      <div
                        ...
                        className={`printable-qr-card p-6 bg-[#FAF6F0] rounded-3xl border-2 border-dashed border-[#8B735B] text-center space-y-4 ${
                          idx < tables.length - 1 ? 'page-break-after' : ''
                        }`}
                      >
                        <span className="inline-block px-3 py-1 rounded-full bg-amber-200 text-amber-900 text-[10px] font-bold uppercase tracking-wider">
                          {tbl.name} • Photo Drop
                        ...
                        <div className="w-40 h-40 mx-auto bg-white p-3 rounded-2xl border-2 border-[#CBAE94] shadow-md flex items-center justify-center">
                          <img
                            src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(
                              `${window.location.origin}/upload-photos?tableId=${tbl.id}`
                            )}`}
                        ...
                        <div className="text-[10px] font-mono text-[#8B735B] pt-2 border-t border-dashed border-[#CBAE94]/60">
                          Table: {tbl.name} • Scan & Upload • No App Required
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2 print:hidden">
                <button
                  type="button"
                  onClick={() => setIsTableQrModalOpen(false)}
